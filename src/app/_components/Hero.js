    ...
    return (
        <div className={`relative flex flex-wrap items-end justify-between w-full gap-12 px-16  py-96`}>
            ...
            <div className='relative z-50 flex w-[30rem] flex-col gap-8'>
                ...
                <Link href={`#${value}`} className='text-secondary text-base leading-[4rem]  font-semibold tracking-[-1px] cursor-pointer'>{buttonText} <span  dangerouslySetInnerHTML={{
        __html: ` ${arabic ? "←" : "→"}`
      }}/> </Link>
            </div>
